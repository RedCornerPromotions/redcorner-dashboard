...
class AWSMediaLiveManager {
    constructor() {
        ...
        console.log('[AWS MediaLive Manager] Initialized');
    ...
    /**
     * Configure RTMP destination for a channel
     * Channel must be IDLE to update configuration
     */
    async configureRTMPDestination(channelNumber, rtmpUrl, streamKey, destinationName = 'RTMP-Output') {
        const channelId = this.channelMap[channelNumber];
        ...
        try {
            ...
            const filteredOutputGroups = outputGroups.filter(og => {
                const name = og.Name || '';
                return !name.includes('RTMP') && !name.includes('SRT') && !name.includes('rtmp') && !name.includes('srt');
            });
            ...
            const rtmpOutputGroup = {
                ...
                Outputs: [{
                    ...
                    VideoDescriptionName: currentConfig.EncoderSettings.VideoDescriptions[0].Name,
    ...
    /**
     * Configure SRT destination for a channel
     */
    async configureSRTDestination(channelNumber, srtUrl, streamId, destinationName = 'SRT-Output') {
        const channelId = this.channelMap[channelNumber];
        ...
        try {
            ...
            const srtOutputGroup = {
                ...
                Outputs: [{
                    ...
                    OutputSettings: {
                        UdpOutputSettings: {
                            Destination: {
                                DestinationRefId: 'srt-destination'
                            },
                            ContainerSettings: {
                                M2tsSettings: {
            ...
            filteredOutputGroups.push(srtOutputGroup);

            const srtDestination = {
                Id: 'srt-destination',
                Settings: [{
                    Url: `udp://${host}:${port}?pkt_size=1316&mode=caller&streamid=${encodeURIComponent(streamId)}`
                }]
            };

            const updatedDestinations = destinations.filter(d => d.Id !== 'srt-destination');
            updatedDestinations.push(srtDestination);
            ...
            console.log(`[AWS MediaLive] Configured SRT destination for channel ${channelNumber}: ${srtUrl}`);
            ...
            console.error(`[AWS MediaLive] Error configuring SRT destination for channel ${channelNumber}:`, error.message);
            return {
                success: false,
                error: error.message
            };
    ...
    async removeDestination(channelNumber) {
        ...
        try {
            ...
            const filteredDestinations = destinations.filter(d =>
                d.Id !== 'rtmp-destination' && d.Id !== 'srt-destination'
            );
                ...
                Creates: {
                    ScheduleActions: [{
                        ...
                        ScheduleActionSettings: {
                            MotionGraphicsImageActivateSettings: { Url: overlayUrl, Duration: 86400000 }
                        }
                    }]
                }
            });
